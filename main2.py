...
from src.idnetmodels.idnet import IDEDEQIDO
import os
import time
...
def main(args: DictConfig):
    ...
    model = IDEDEQIDO(args.id_4x).to(device)

    # load from pretrain model
    ckpt = torch.load(args.id_4x.pretrain_ckpt, map_location=device)
    ...
    for epoch in range(args.train.epochs):
        ...
        for i, batch in enumerate(tqdm(train_data)):
            ...
            ground_truth_flow = batch["flow_gt"].float().to(device) # [B, 2, 480, 640]
            flow = model(event_image) # [B, 2, 480, 640]
            loss: torch.Tensor = compute_epe_error(flow["final_prediction"], ground_truth_flow)
